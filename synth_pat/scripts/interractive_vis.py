...
def pivot_metric(metric):
    return feat_df.pivot(columns="we", index="sigma", values=metric)
...
we_vals = varfcd_map.columns.values
sigma_vals = varfcd_map.index.values
...
ax_alff_stg.set_title("Sweep R.STG_ALFF")
...
xticks = np.arange(len(we_vals))
yticks = np.arange(len(sigma_vals))
xlabels = [f"{v:.3g}" for v in we_vals]
ylabels = [f"{v:.3g}" for v in sigma_vals]
...
def update_plots(we_value, sigma_value):
    ...
    # find indices
    col_idx = np.where(we_vals == we_value)[0][0]
    row_idx = np.where(sigma_vals == sigma_value)[0][0]
    ...
    idx = np.intersect1d(
        np.where(feat_df['we'] == we_value),
        np.where(feat_df['sigma'] == sigma_value)
    )[0]
...
def on_click(event):
    if event.inaxes in [ax_varfcd, ax_meanfc, ax_alff_ca, ax_alff_stg]:
        ...
        if 0 <= x < len(we_vals) and 0 <= y < len(sigma_vals):
            we_value = we_vals[x]
            sigma_value = sigma_vals[y]
            update_plots(we_value, sigma_value)
